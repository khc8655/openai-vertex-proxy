...
async function handleModels () {
  const body = JSON.stringify({
    object: "list",
    data: [
      { id: "google/gemini-2.5-flash", object: "model", created: 1720000000, owned_by: "google" },
      { id: "google/gemini-3.5-flash", object: "model", created: 1720000000, owned_by: "google" },
      { id: "google/imagen-4.0-ultra-generate-001", object: "model", created: 1720000000, owned_by: "google" },
      { id: "google/imagen-4.0-fast-generate-001", object: "model", created: 1720000000, owned_by: "google" }
    ]
  }, null, "  ");
  ...
  if (response.ok) {
    ...
    if (req.stream) {
      ...
      try {
        ...
        responseBody = processCompletionsResponse(parsed, "google/" + model, id);
      } catch (err) {
        console.error("Error parsing response:", err, rawText);
        return new Response(rawText, fixCors(response));
      }
    }
  } else {
  ...
  if (req.reasoning_effort) {
    cfg.thinkingConfig =
      isV3
        ...
        : { thinkingBudget: thinkingBudgetMap[req.reasoning_effort] ?? 8192 };
  }
  return cfg;
};
...
function transformCandidates (key, cand) {
  ...
  for (const part of cand.content?.parts ?? []) {
    if (part.functionCall) {
      ...
      if (part.thought === true) {
        if (isDelta) {
          payload.reasoning_content = (payload.reasoning_content ?? "") + part.text;
        } else {
          reasoningText += part.text;
        }
      } else {
        if (isDelta) {
          payload.content = (payload.content ?? "") + part.text;
        } else {
  ...
  return {
    ...
    [key]: payload,
    logprobs: null,
    finish_reason: tool_calls ? "tool_calls" : reasonsMap[cand.finishReason] ?? cand.finishReason,
  };
}
